...
# ============================================================================
# Card Models (HU-015: Card Management)
# ============================================================================

class CardType(Enum):
    """
    Card type enumeration - Value Object
    Immutable by design (Enum protocol)
    """
    DEBIT = 'DEBIT'
    CREDIT = 'CREDIT'
...
class CardStatus(Enum):
    """
    Card status enumeration - Value Object
    Immutable by design (Enum protocol)
    
    States:
    - ACTIVE: Card is usable
    - INACTIVE: Card has been soft-deleted (preserved for audit trail)
    """
    ACTIVE = 'ACTIVE'
    INACTIVE = 'INACTIVE'
...
class Card:
    """
    Card Entity - Represents a payment card (debit/credit)
    
    Immutable (frozen=True):
    - Guarantees thread-safety
    - Enforces DDD discipline (Value Object-like semantics for immutability)
    - Prevents accidental mutations
    
    Note: card_id and timestamps auto-generated on creation
    """

    # Core card data
    card_number: str  # 16-digit card number (stored as plain text in MVP)
    card_holder_name: str  # Name on card (3-50 chars)
    expiry_date: str  # MM/YY format
    card_type: str  # DEBIT or CREDIT (from CardType enum)
    user_id: str  # FK to user who owns this card

    # Optional metadata
    nickname: Optional[str] = None  # User-friendly name (e.g., "My Main Card")
    status: str = 'ACTIVE'  # ACTIVE or INACTIVE (soft delete)

    # Audit fields (auto-set)
    card_id: str = field(default_factory=lambda: '')  # Will be set by repository
    created_at: datetime = field(default_factory=datetime.utcnow)
    updated_at: datetime = field(default_factory=datetime.utcnow)

    def __post_init__(self) -> None:
        """
        Validation hook - called after __init__
        Cannot modify fields after this due to frozen=True
        """
        # Validation is delegated to card_validators module
        # This keeps domain model focused on data structure
        pass
    ...
    def mask_card_number(self) -> str:
        """Return masked card number for display (****0366)"""
        return f"****{self.get_last_four_digits()}"
    ...
    def is_debit(self) -> bool:
        ...
        return self.card_type == 'DEBIT'

    def is_credit(self) -> bool:
        """Check if this is a credit card"""
        return self.card_type == 'CREDIT'
